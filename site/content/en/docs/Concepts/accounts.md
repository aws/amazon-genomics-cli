...
---
AGC requires an AWS account in which to deploy the cloud infrastructure required to run and manage workflows. To begin
working with AGC and account must be "Activated" by the AGC application using the [account activate]( {{< relref "#activate" >}}) command.

## Which AWS Account is Used by AGC?

AGC uses the same [AWS credential chain](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-quickstart.html#cli-configure-quickstart-precedence) 
...
## Which Region is Used by AGC?

Much like accounts and credentials AGC uses the same chain used by the AWS CLI to determine the region that is being targeted.
For example, if your AWS profile uses `us-east-1` then AGC will use the same. Likewise, if you set the `AWS_REGION` environment
variable to `eu-west-1` then that region will be used by AGC for all subsequent commands in that shell.

## Shared Infrastructure

When a region is first activated for AGC some basic infrastructure is deployed including a [VPC](https://docs.aws.amazon.com/vpc/latest/userguide/index.html) 
and an [S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/index.html) bucket. This
core infrastructure will be shared by all AGC users and projects in that region. Note that context specific infrastructure
is not shared and is unique and namespaced by user and project.

## Bring your Own VPC and S3 Bucket

During account [activation]( {{< relref "#activate" >}}) you may specify an existing VPC ID or S3 bucket name for use by AGC. If you do not these will 
be created for you. Although we use AWS best practices for these, if your organization has specific security requirements 
for networking and storage this may be the easiest way to activate AGC in your environment.
...
AGC requires an S3 bucket to store workflow results and associated information. If you prefer to use an existing bucket
...
AGC must be able to write to that bucket.
...
such as via a NAT gateway, and connectivity to AWS services either through VPC endpoints or the internet. AGC will not
...
`agc account activate --bucket my-existing-bucket --vpc my-existing-vpc-id` then AGC will update to use `my-existing-bucket`
...
and later invoked `agc account activate` then AGC will stop using the previous specified bucket and VPC. *ALL* of the 
pre-existing S3 and VPC infrastructure will be retained and a new bucket and VPC will be created for use by AGC.

### `deactivate`

The `deactivate` command is used to remove the core infrastructure deployed by AGC in the current region when an 
account is activated. The S3 bucket deployed by AGC  and its contents are retained. If a VPC and/ or S3 bucket were 
specified by the user during account activation these will also be retained. Any CloudWatch logs produced by AGC will
also be retained.
...
If the deployed infrastructure has been modified through the console or the AWS CLI rather than through AGC deactivation
...
Core infrastructure deployed for AGC is [tagged]( {{< relref "namespaces#tags" >}} ) with the `application-name: agc` tag. This tag can be activated for cost
...
as VPC NAT gateways and VPC Endpoints. If you no longer use AGC in a region we recommend you deactivate it. You may also
wish to remove the S3 bucket along with its objects as well as the [CloudWatch](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/index.html) logs produced by AGC. These are retained
...
the use of VPC Enpoints [(see below)]( {{< relref "#VPC Endpoints" >}} ) as well as activating AGC and running your workflows in the same region as your S3
...
When AGC creates a VPC it creates the following VPC endpoints:
...
AGC in your VPC and will also reduce NAT gateway traffic charges which can be substantial for genomics analyses that use
large S3 objects and/ or large container images.

## Technical Details.

AGC core infrastructure is defined in code and deployed by [AWS CDK](https://aws.amazon.com/cdk/). The CDK app responsible for creating the core
