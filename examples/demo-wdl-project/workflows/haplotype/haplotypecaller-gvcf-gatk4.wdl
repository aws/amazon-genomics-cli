...
workflow HaplotypeCallerGvcf_GATK4 {
  input {
    ...
    String gitc_docker = "public.ecr.aws/aws-genomics/broadinstitute/genomes-in-the-cloud:2.4.7-1603303710"
...
task HaplotypeCaller {
  ...
  command {
    ...
    ~{gatk_path} --java-options "-Xmx~{command_mem_gb}G ~{java_opt}"       HaplotypeCaller       -R ~{ref_fasta}       -I ~{input_bam}       -L ~{interval_list}       -O ~{output_filename}       -contamination ~{default="0" contamination}       -G StandardAnnotation -G StandardHCAnnotation ~{true="-G AS_StandardAnnotation" false="" make_gvcf}       -GQB 10 -GQB 20 -GQB 30 -GQB 40 -GQB 50 -GQB 60 -GQB 70 -GQB 80 -GQB 90       ~{true="-ERC GVCF" false="" make_gvcf}       ~{if defined(gcs_project_for_requester_pays) then "--gcs-project-for-requester-pays ~{gcs_project_for_requester_pays}" else ""}       ~{bamout_arg}
...
task MergeGVCFs {
  ...
  set -e
    ...
    ~{gatk_path} --java-options "-Xmx~{command_mem_gb}G"        MergeVcfs       --INPUT ~{sep=' --INPUT ' input_vcfs}       --OUTPUT ~{output_filename}
  }
  runtime {
    docker: docker
    memory: machine_mem_gb + " GB"
