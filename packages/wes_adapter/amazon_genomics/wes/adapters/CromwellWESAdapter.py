...
class CromwellWESAdapter(AbstractWESAdapter):  # inherit from ABC to enforce interface
    ...
    def run_workflow(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if u.scheme == "s3":
                ...
                try:
                    props = self.get_workflow_from_s3(
                        workflow_url, tmpdir, workflow_type
                    )
    ...
    def get_workflow_from_s3(self, s3_uri: str, localpath: str, workflow_type: str):
        """
        ...
        * optionally, if an `options.json` file is found in the root level of the zip, it will be set as `workflowOptions`
        ...
        if path.basename(file) == "workflow.zip":
            try:
                props = self.parse_workflow_zip_file(file, workflow_type)
    ...
    def parse_workflow_zip_file(self, file, workflow_type):
        """
        ...
        with zipfile.ZipFile(file) as zip:
            ...
            if len(contents) == 1:
                ...
                if "MANIFEST.json" in contents:
                    props = self.parse_workflow_manifest_file(
    ...
    def parse_workflow_manifest_file(self, manifest_file):
        """
            ...
            for url in manifest["inputFileURLs"]:
                ...
                if not u.scheme or u.scheme == "file":
                    files[f"workflowInputFiles"] += [
                        open(
                            workflow_manifest_url_to_path(
                                u, path.dirname(manifest_file)
                            ),
                            "rb",
                        )
                    ]
