...
must be running on a computer with access to the VPC network, such as an EC2 within the VPC.`
	...
	REGIONAL_ENDPOINT_TYPE = "REGIONAL"
	PRIVATE_ENDPOINT_TYPE  = "PRIVATE"
)
...
func (o *accountActivateOpts) validate() error {
	...
	if o.endpointType != REGIONAL_ENDPOINT_TYPE && o.endpointType != PRIVATE_ENDPOINT_TYPE {
		return o.generateValidationError(fmt.Errorf("invalid endpointType '%s', endpointType must be one of %s or %s", o.endpointType, REGIONAL_ENDPOINT_TYPE, PRIVATE_ENDPOINT_TYPE),
...
/code $ agc account activate --bucket my-custom-bucket --vpc my-vpc-id`,
	...
	cmd.Flags().StringVar(&vars.endpointType, endpointTypeFlag, REGIONAL_ENDPOINT_TYPE, endPointTypeFlagDescription)
	return cmd
}
