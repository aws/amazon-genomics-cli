	...
	testCases := map[string]struct {
		...
		"multiple deploy success": {
			...
			setupMocks: func(t *testing.T) mockClients {
				...
				mockClients.cdkMock.EXPECT().DeployApp(filepath.Join(testHomeDir, ".agc/cdk/apps/context"), gomock.Len(42), testContextName1).After(clearContext).Return(mockClients.progressStream1, nil)
				mockClients.cdkMock.EXPECT().DeployApp(filepath.Join(testHomeDir, ".agc/cdk/apps/context"), gomock.Len(42), testContextName2).After(clearContext2).Return(mockClients.progressStream2, nil)
				displayProgressBar = mockClients.cdkMock.DisplayProgressBar
		...
		"deploy error": {
			...
			setupMocks: func(t *testing.T) mockClients {
				...
				mockClients.ssmMock.EXPECT().GetCustomTags().Return(testTags)
				mockClients.cdkMock.EXPECT().ClearContext(filepath.Join(testHomeDir, ".agc/cdk/apps/context")).Return(nil)
				mockClients.cdkMock.EXPECT().DeployApp(filepath.Join(testHomeDir, ".agc/cdk/apps/context"), gomock.Len(42), testContextName1).Return(nil, fmt.Errorf("some context error"))
